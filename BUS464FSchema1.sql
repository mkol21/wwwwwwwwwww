...
DROP SCHEMA IF EXISTS gap_retail_db;
CREATE SCHEMA IF NOT EXISTS gap_retail_db;
USE gap_retail_db;
...
-- Use Case 1: Online Order Processing
DELIMITER $$
CREATE PROCEDURE ProcessOnlineOrder(
    -- Product Selection
    IN p_variant_id INT,
    IN p_quantity INT,
    
    -- Delivery Method
    IN p_delivery_method ENUM('Standard Shipping', 'Express Shipping', 'Store Pickup'),
    IN p_store_id INT,
    ...
    -- Output
    OUT p_order_id INT,
    OUT p_order_status VARCHAR(50),
    OUT p_confirmation_number VARCHAR(100)
)
BEGIN
...
-- Use Case 2: In-Store Order Processing
DELIMITER $$
CREATE PROCEDURE ProcessInStoreOrder(
    IN p_store_id INT,
    IN p_customer_id INT,
    IN p_items JSON,  -- Array of {variant_id, quantity}
    IN p_payment_method_id INT,
    IN p_payment_details JSON,
    IN p_promo_code VARCHAR(20),
    IN p_use_loyalty_points BOOLEAN,
    OUT p_order_id INT,
    OUT p_receipt_number VARCHAR(100)
)
BEGIN
    -- Implementation for in-store orders
END $$
DELIMITER ;
...
CREATE PROCEDURE ManageInventory(
    IN p_action ENUM('ADD_STOCK', 'REMOVE_STOCK', 'TRANSFER_STOCK', 'UPDATE_THRESHOLD'),
    IN p_variant_id INT,
    IN p_quantity INT,
    IN p_location_type ENUM('Store', 'Warehouse'),
    IN p_location_id INT,
    IN p_destination_id INT,  -- For transfers
    IN p_new_threshold INT,  -- For threshold updates
...
-- Use Case 4: Warehouse Fulfillment and Shipping
DELIMITER $$
CREATE PROCEDURE ProcessFulfillment(
    IN p_order_id INT,
    IN p_warehouse_id INT,
    IN p_shipment_type VARCHAR(50),
...
-- Use Case 5: Returns/Refunds
DELIMITER $$
CREATE PROCEDURE ProcessReturn(
    IN p_order_id INT,
    IN p_return_reason TEXT,
    IN p_return_items JSON,  -- Array of {order_item_id, quantity, is_damaged}
    IN p_refund_method_id INT,
    OUT p_return_id INT,
...
-- Use Case 6: Promotion Application
DELIMITER $$
